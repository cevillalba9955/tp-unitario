...
export default function FreelancerOrderDetailScreen({ route }) {
  ...
  const confirmReject = () => {
    Alert.alert('Rechazar pedido', '¿Seguro que querés rechazar este pedido?', [
      { text: 'No', style: 'cancel' },
      { text: 'Sí, rechazar', style: 'destructive', onPress: () => act('reject') },
    ]);
  };
  ...
  const isPendiente = order.stage === 'PENDIENTE';
  const isConfirmado = order.stage === 'CONFIRMADO';
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      {isPendiente && (
        <>
          <TouchableOpacity style={styles.primaryButton} disabled={acting} onPress={() => act('accept')}>
            <Text style={styles.primaryButtonText}>Aceptar</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.dangerButton} disabled={acting} onPress={confirmReject}>
      ...
      {isConfirmado && (
        <TouchableOpacity style={styles.primaryButton} disabled={acting} onPress={() => act('deliver')}>
          <Text style={styles.primaryButtonText}>Entregar trabajo</Text>
        </TouchableOpacity>
      )}
    </ScrollView>
  );

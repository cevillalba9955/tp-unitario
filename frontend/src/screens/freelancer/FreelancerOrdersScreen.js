...
export default function FreelancerOrdersScreen({ navigation }) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await getOrders('freelancer');
      setOrders(data);
    } catch {
      setError('No se pudieron cargar tus pedidos entrantes.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const renderItem = ({ item }) => (
    <TouchableOpacity
      style={styles.card}
      onPress={() => navigation.navigate('FreelancerOrderDetail', { orderId: item.id })}
    >
  ...
  return (
    <FlatList
      ...
      ListEmptyComponent={<Text style={styles.emptyText}>Aún no tenés pedidos entrantes.</Text>}
    />
  );
}
